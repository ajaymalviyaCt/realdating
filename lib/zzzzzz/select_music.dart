...
class _SelectMusicState extends State<SelectMusic> {
  ...
  Widget segmentView() {
    return HorizontalMenuBar(
        ...
        menus: _createReelController.categories.map((element) => element.name).toList());
  }
  ...
  void openCropAudio(ReelMusicModel audio) async {
    ...
    File audioFile = await Get.bottomSheet(FractionallySizedBox(heightFactor: 0.7, child: CropAudioScreen(reelMusicModel: audio)));
